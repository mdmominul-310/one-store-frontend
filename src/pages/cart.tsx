...
const Cart: React.FC = (): JSX.Element => {
    ...
    return (
        <div className="container my-4 min-h-screen">
            <h2 className="my-5 text-3xl pb-2 font-semibold border-b-[1px]">Your Cart</h2>
            <div className="grid grid-cols-12 gap-4">
                ...
                <div className="grid lg:col-span-4 col-span-12 h-[270px] bg-gray-100 p-3">
                    <div className=" shadow p-3 rounded bg-white ">
                        <h5 className="text-center mb-3 pb-2 text-lg font-semibold border-b-[1px]">Order Summary</h5>
                        <h4 className="flex justify-between my-2"><span>Subtotal ({totalSelectedItems} items)</span> <span>${totalSelectedPrice}</span></h4>
                        <div className="flex">
                            <Input placeholder="Enter Coupon" type="text" className="h-10" />
                            <Button className="bg-primary   ml-2 text-white" >Apply</Button>
                        </div>
                        <h4 className="flex justify-between my-2"><span>Total</span><span>${totalSelectedPrice}</span></h4>
                        <button className="bg-primary font-semibold text-white rounded hover:bg-secondary duration-300 p-2 w-full my-2" onClick={handleCheckout}>PROCEED TO CHECKOUT ({totalSelectedPrice})</button>
                    </div>
                </div>
            </div>
        </div>
