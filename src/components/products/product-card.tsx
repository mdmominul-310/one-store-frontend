    ...
    return (
        <>
            <div className="shadow-lg  rounded-lg relative group bg-white "  >
                <div onClick={handleproduct}>
                    <div className="overflow-hidden">
                        <img src={products?.images?.[0]} alt="product" className="hover:scale-125 transition duration-500 rounded-t-lg" />
                    </div>
                    <div className="px-2">
                        {/* product title  */}
                        <h3 className="font-[500] text-md  hover:text-primary">{products?.title}</h3>
                        <div className="flex justify-between font-semibold text-primary">
                            {/* product sale price */}
                            <p>৳{products?.stock?.[0]?.salePrice}</p>
                            <p><del>{products?.stock?.[0]?.regularPrice}৳</del></p>
                        </div>
                    </div>
                    <div className="bg-primary absolute top-1 left-0 px-2 py-1 text-xs rounded text-white font-semibold">
                        save {
                            Math.floor(((parseInt(products?.stock?.[0]?.regularPrice) - parseInt(products?.stock?.[0]?.salePrice)) / parseInt(products?.stock?.[0]?.regularPrice)) * 100)
                        } %
                    </div>
                </div>
                ...
                <div className="py-2 px-2">
                    <button className="bg-primary w-full font-semibold text-white hover:bg-secondary py-1 rounded"
                        onClick={() => setIsModalVisible(true)}
                    >Add Cart</button>
                </div>
            </div>
            ...
            <Modal
                ...
                width={1000}
            >
                <ProductViewCard product={products} setIsModalVisible={setIsModalVisible} />
            </Modal>
        </>
    )
}
