...
const ProductViewCard = ({ product, setIsModalVisible }: { product: IProducts, setIsModalVisible?: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const handleAttibutes = (name: string, value: string) => {
        ...
        if (isNameExist) {
            setAttibutes(attibutes.map(att => att.key === name ? { ...att, value } : att));
        } else {
    ...
    return (
        ...
        <div className=" block  lg:flex justify-between gap-4 overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 inline-block shadow md:p-4 basis-1/2">
                ...
                {/* sku  */}
                <h4 className=" text-xs text-gray-400">SKU: {product.stock?.[0].sku} </h4>
                <h3 className="flex  items-baseline gap-2 text-primary border-b-2 pb-4 mb-4 mt-2">
                    {/* sale price  */}

                    <span className="text-3xl font-normal ">TK.{product.stock?.[0].salePrice} </span>
                    <p className="font-semibold  text-secondary flex "><del>TK.{product.stock?.[0].regularPrice} </del></p>
                </h3>
                                            ...
                                            attr.values.map((value, index) => {
                                                ...
                                                return (
                                                    <span className={cn(
                                                        ...
                                                        { 'bg-primary text-white': selectedAtt?.value === value.title }
                                                    )}
                ...
                <div className="flex text-white gap-4 my-4">
                    ...
                    <div className="flex justify-center items-center md:gap-6 gap-1 ">
                        <div className="border-[1px] rounded flex items-center border-primary">
                            <button className="px-4 py-1 text-xl text-black hover:bg-primary hover:text-white border-r-[1px] border-primary"
                                onClick={() => qty > 1 ? setQty(qty - 1) : null}
                            >
                                -
                            </button>
                            <span className="text-black w-[40px] text-center">{qty}</span>
                            <button className="px-4 py-1 text-xl text-black  hover:bg-primary hover:text-white border-l-[1px] border-primary"
                    ...
                    <Button className="w-full border-[1px] bg-white text-black hover:text-white border-primary hover:bg-primary"
                        onClick={handleCart}
                    >Add To Cart</Button>
                ...
                <div className="flex gap-2">
                    ...
                    <div className="flex gap-4">
                        {product.tags?.map((tag: { title: string, label: string }, index: number) => (
                            <span className="bg-primary px-1 rounded text-white hover:bg-secondary" key={index}>{tag.title}</span>
