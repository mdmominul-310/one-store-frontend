    ...
    return (
        <div className="my-4 ">
            <div className="border-b-2  flex flex-col items-center">
                <h2 className='text-xl font-medium text-center uppercase '>Browse Our Categories</h2>
            ...
            <div className="">
                <div className="slider-container">
                    <Slider ref={sliderRef} {...settings} >
                        {
                            data?.data?.map((item: { id: string, name: string, image?: string }, index: number) => (
                                <div key={index} className=" p-3 relative cursor-pointer" onClick={() => navigate(`/categories/${item.name}`)} >
                                    <img src={item?.image ? item.image : "https://cdn.bitcommerz.com/manfare-bd/media/1710417915870-manfare_bd-id-13.jpeg"} alt="" className="rounded-lg" />
