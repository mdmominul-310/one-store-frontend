...
const CartItems = ({ cartItems, flat }: CartItemsProps) => {
    const { addQuantity, reduceQuantity, countDiscount, removeToCart } = useCart();
            ...
            <div className="bg-slate-50 shadow my-2 p-3 w-full flex rounded    duration-300">
                ...
                <div className="w-full">
                    <div className="flex gap-6">
                        <div >
                            <img src={cartItems?.image} className={cn("w-[150px]", { "w-[50px]": flat })} alt="" />
                        </div>
                        <div className={cn("flex flex-col gap-2", { "block": flat })}>
                            ...
                            <div className="flex items-baseline gap-1 font-semibold text-md">
                                ...
                                <span className={cn("my-1 block", { "block": flat })}>
                                    <del className="text-xs text-rose-600">৳{cartItems.regularPrice}</del>
                                </span>
                            ...
                            <div className={
                                cn(
                                    "md:flex gap-4 items-center font-semibold",
                                    { 'flex': flat }
                                )
                            }>
                                {
                                    flat ? (
                                        ...
                                        <div className="border-[1px] w-[120px]  flex items-center border-slate-200">
                                            <button className="px-4 py-1 text-xl  hover:bg-primary hover:text-white border-r-[1px] border-slate-200"
                                                onClick={() => reduceQuantity(cartItems.id)}>-</button>
                                            <p className=" text-center w-[40px]">{cartItems?.qty}</p>
                                            <button className=" px-4 py-1 text-xl  hover:bg-primary hover:text-white border-l-[1px] border-slate-200 "
                                                onClick={() => addQuantity(cartItems.id)}>+</button>
                                        </div>
                                    )
                                }
                                {
                                    flat ? (
                                        <div className="md:flex  gap-1 text-xl items-center my-2">
                                            ...
                                            >Save</button>
                                            <button className=" px-2 mt-2 md:mt-0 py-1 text-[15px]  font-light   hover:bg-primary hover:text-white border-[1px] border-slate-200"
                                        ...
                                        (
                                            <div className="md:flex  gap-4 text-xl items-center my-2">
                                                ...
                                                >Save</button>
                                                <button className="md:px-10 px-8 mt-2 md:mt-0 py-1 text-[15px]  font-light   hover:bg-primary hover:text-white border-[1px] border-slate-200"
                                                    onClick={() => removeToCart(cartItems.id)}
                                                >Remove</button>
