...
const DrawerCartContent = ({ carts, setShowDrawer }: { carts: ICarts[], setShowDrawer: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    return (
        <div className="">
            ...
            <div className="absolute bottom-4 w-ful left-32">
                <button className="bg-primary flex gap-2 w-full p-2 rounded text-white" onClick={handleCheckout}>
                    <h3 className="font-semibold ">Checkout</h3>
                    <h3 className="font-semibold ">Total: ৳{carts.reduce((acc, item) => acc + item.price * item.qty, 0)}</h3>
                </button>
            </div>
