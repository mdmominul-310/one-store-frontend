...
const NavBarMobile = () => {
    ...
    return (
        <>
            ...
            <Drawer title="Items" onClose={onClose} open={open} placement="left" width={200}>
                {isLoading ? <div>Loading...</div> : (
                    <div>
                        {
                            data?.data?.map((item: { _id: string, name: string, children: [{ label: string, title: string, _id: string }] }) => (
                                <div key={item._id}>
                                    <DropDown label={item.name} items={item?.children?.map((subItem) => ({
                                        key: subItem._id,
                                        label: (
